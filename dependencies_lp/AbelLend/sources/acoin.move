module abel::acoin {
    ...
    public fun balance_no_type_args(coin_type: String, owner: address): u64 acquires AccountSnapshotTable {
        let account_snapshots_ref = &borrow_global<AccountSnapshotTable>(owner).account_snapshots;
    ...
    public fun is_account_registered_no_type_args(coin_type: String, account_addr: address): bool acquires AccountSnapshotTable {
        if (!exists<AccountSnapshotTable>(account_addr)) {
            false
        } else {
            let account_snapshots_ref = &borrow_global<AccountSnapshotTable>(account_addr).account_snapshots;
            table::contains<String, ACoinUserSnapshot>(account_snapshots_ref, coin_type)
        }
    }
    ...
    public fun total_supply_no_type_args(coin_type: String): u128 acquires GlobalSnapshotTable {
        ...
        acoin_global_snapshot_ref.total_supply
    }

    public fun total_borrows<CoinType>(): u128 acquires ACoinInfo {
        borrow_global<ACoinInfo<CoinType>>(acoin_address()).total_borrows
    }
    public fun total_borrows_no_type_args(coin_type: String): u128 acquires GlobalSnapshotTable {
        let acoin_global_snapshot_ref = table::borrow<String, ACoinGlobalSnapshot>(&borrow_global<GlobalSnapshotTable>(acoin_address()).snapshots, coin_type);
        acoin_global_snapshot_ref.total_borrows
    }

    public fun total_reserves<CoinType>(): u128 acquires ACoinInfo {
        borrow_global<ACoinInfo<CoinType>>(acoin_address()).total_reserves
    }
    public fun total_reserves_no_type_args(coin_type: String): u128 acquires GlobalSnapshotTable {
        let acoin_global_snapshot_ref = table::borrow<String, ACoinGlobalSnapshot>(&borrow_global<GlobalSnapshotTable>(acoin_address()).snapshots, coin_type);
        acoin_global_snapshot_ref.total_reserves
    }

    public fun borrow_index<CoinType>(): u128 acquires ACoinInfo {
        borrow_global<ACoinInfo<CoinType>>(acoin_address()).borrow_index
    }
    public fun borrow_index_no_type_args(coin_type: String): u128 acquires GlobalSnapshotTable {
        let acoin_global_snapshot_ref = table::borrow<String, ACoinGlobalSnapshot>(&borrow_global<GlobalSnapshotTable>(acoin_address()).snapshots, coin_type);
        acoin_global_snapshot_ref.borrow_index
    }
    ...
    public fun reserve_factor_mantissa_no_type_args(coin_type: String): u128 acquires GlobalSnapshotTable {
        let acoin_global_snapshot_ref = table::borrow<String, ACoinGlobalSnapshot>(&borrow_global<GlobalSnapshotTable>(acoin_address()).snapshots, coin_type);
        acoin_global_snapshot_ref.reserve_factor_mantissa
    }
    ...
    public fun get_cash_no_type_args(coin_type: String): u64 acquires GlobalSnapshotTable {
        let acoin_global_snapshot_ref = table::borrow<String, ACoinGlobalSnapshot>(&borrow_global<GlobalSnapshotTable>(acoin_address()).snapshots, coin_type);
        acoin_global_snapshot_ref.treasury_balance
    }
    ...
    public fun borrow_balance_no_type_args(coin_type: String, borrower: address): u64 acquires AccountSnapshotTable, GlobalSnapshotTable {
        let account_snapshots_ref = &borrow_global<AccountSnapshotTable>(borrower).account_snapshots;
        assert!(table::contains<String, ACoinUserSnapshot>(account_snapshots_ref, coin_type), EACOIN_NOT_REGISTERED_BY_USER);
        let acoin_snapshot_ref = table::borrow<String, ACoinUserSnapshot>(account_snapshots_ref, coin_type);
        let principal = acoin_snapshot_ref.borrow_principal;
        let interest_index = acoin_snapshot_ref.interest_index; 
        let global_interest_index = table::borrow<String, ACoinGlobalSnapshot>(&borrow_global<GlobalSnapshotTable>(acoin_address()).snapshots, coin_type).borrow_index;
        
        ((principal as u128) * global_interest_index / interest_index as u64)
    }

    public fun borrow_rate_per_block_no_type_args(coin_type: String): u128 acquires GlobalSnapshotTable {
        let acoin_global_snapshot_ref = table::borrow<String, ACoinGlobalSnapshot>(&borrow_global<GlobalSnapshotTable>(acoin_address()).snapshots, coin_type);
        ...
        interest_rate_module::get_borrow_rate((total_cash as u128), total_borrows, total_reserves)
    }
    ...
    public(friend) fun initialize<CoinType>(
        ...
        table::add<String, ACoinGlobalSnapshot>(snapshots_ref, type_name<CoinType>(), ACoinGlobalSnapshot{
            ...
            accrual_block_number: timestamp::now_seconds(),
    ...
    public(friend) fun update_accrual_block_number<CoinType>() acquires ACoinInfo, GlobalSnapshotTable {
        borrow_global_mut<ACoinInfo<CoinType>>(acoin_address()).accrual_block_number = timestamp::now_seconds();

        // update snapshot
        let snapshots_ref = &mut borrow_global_mut<GlobalSnapshotTable>(acoin_address()).snapshots;
        table::borrow_mut<String, ACoinGlobalSnapshot>(snapshots_ref, type_name<CoinType>()).accrual_block_number = timestamp::now_seconds();
    }

module abel::market {
    ...
    use abel::oracle;
    use abel::constants::{Self, Exp_Scale, Half_Scale};
    ...
    // claim abel
    public entry fun claim_abel(holder: address) {
        ...
        while (index < market_len) {
            ...
            if (acoin::is_account_registered_no_type_args(coin_type, holder)) {
                ...
                update_abelcoin_borrow_index_no_type_args(coin_type, borrow_index);
                distribute_borrower_abelcoin_no_type_args(coin_type, holder, borrow_index, true);

                update_abelcoin_supply_index_no_type_args(coin_type);
                distribute_supplier_abelcoin_no_type_args(coin_type, holder, true);
            };
            index = index + 1;
        };
    ...
    // ABEL admin functions
    public entry fun fund_abel_treasury(funder: &signer, amount: u64) {
        storage::deposit_abel(coin::withdraw<AbelCoin>(funder, amount));
    }

    public entry fun set_abel_rate(admin: &signer, abel_rate: u128) {
        only_admin(admin);
        storage::set_abel_rate(abel_rate);
        refresh_abel_speeds();
    }

    public entry fun add_abel_market<CoinType>(admin: &signer) {
        only_admin(admin);
        storage::add_abel_market(type_name<CoinType>());
        refresh_abel_speeds();
    }

    public entry fun drop_abel_market<CoinType>(admin: &signer) {
        only_admin(admin);
        storage::drop_abel_market(type_name<CoinType>());
        refresh_abel_speeds();
    }

    // ABEL Distribution
    public entry fun refresh_abel_speeds() {
        let all_markets = storage::all_markets();
        let market_len = vector::length<String>(&all_markets);
        ...
        while (index < market_len) {
            let coin_type = *vector::borrow<String>(&all_markets, index);
            if (storage::is_abeled_no_type_args(coin_type)) {
        ...
        while (index < market_len) {
            ...
            let new_speed = if (total_utility == 0) {
                ...
                storage::abel_rate() * utility / total_utility
            };
            storage::update_abel_speed(coin_type, new_speed);
            index = index + 1;
        };
    }
    public fun claimable_borrower_abelcoin(coin_type: String, borrower: address): u128 {
        ...
        update_abelcoin_borrow_index_no_type_args(coin_type, market_borrow_index);
        let (borrow_index, _) = storage::abel_borrow_state(coin_type);
        let borrower_index = storage::abel_borrower_index(coin_type, borrower);
    ...
    public fun claimable_supplier_abelcoin(coin_type: String, supplier: address): u128 {
        if (!acoin::is_account_registered_no_type_args(coin_type, supplier)) {
            return 0
        };
        update_abelcoin_supply_index_no_type_args(coin_type);
        let (supply_index, _) = storage::abel_supply_state(coin_type);
        let supplier_index = storage::abel_supplier_index(coin_type, supplier);
    ...
    public fun claimable_abelcoin_each(coin_type: String, holder: address): u128 {
        if (!acoin::is_account_registered_no_type_args(coin_type, holder)) {
            return 0
        };
        claimable_borrower_abelcoin(coin_type, holder) + claimable_supplier_abelcoin(coin_type, holder)
    }
    public fun claimable_abelcoin(holder: address): u128 {
        let all_markets = storage::all_markets();
        let market_len = vector::length<String>(&all_markets);
        let claimable_abelcoin: u128 = storage::abel_accrued(holder);
        let index = 0;
        while (index < market_len) {
            let coin_type = *vector::borrow<String>(&all_markets, index);
            if (acoin::is_account_registered_no_type_args(coin_type, holder)) {
                claimable_abelcoin = claimable_abelcoin + claimable_abelcoin_each(coin_type, holder);
            };
            index = index + 1;
        };
        claimable_abelcoin
    ...
    fun update_abelcoin_supply_index<CoinType>() {
        update_abelcoin_supply_index_no_type_args(type_name<CoinType>());
    }
    fun distribute_supplier_abelcoin<CoinType>(supplier: address, distribute_all: bool) {
        distribute_supplier_abelcoin_no_type_args(type_name<CoinType>(), supplier, distribute_all);
    }
    fun update_abelcoin_borrow_index<CoinType>(market_borrow_index: u128) {
        update_abelcoin_borrow_index_no_type_args(type_name<CoinType>(), market_borrow_index);
    }
    fun distribute_borrower_abelcoin<CoinType>(borrower: address, market_borrow_index: u128, distribute_all: bool) {
        distribute_borrower_abelcoin_no_type_args(type_name<CoinType>(), borrower, market_borrow_index, distribute_all);
    }

    fun update_abelcoin_supply_index_no_type_args(coin_type: String) {
        let (supply_state_index, supply_state_block) = storage::abel_supply_state(coin_type);
        let supply_speed = storage::abel_speed(coin_type);
        ...
        if (delta_blocks > 0 && supply_speed > 0) {
            ...
            let abel_accrued = (delta_blocks as u128) * supply_speed;
            let ratio = if (supply_tokens > 0) {
                abel_accrued * Exp_Scale() / supply_tokens
            } else { 0 };
            let new_index = supply_state_index + ratio;
            storage::update_abel_supply_state(coin_type, new_index, block_number);
        } else if (delta_blocks > 0) { 
            storage::update_abel_supply_state(coin_type, supply_state_index, block_number);
        };
    }
    fun distribute_supplier_abelcoin_no_type_args(coin_type: String, supplier: address, distribute_all: bool) {
        let (supply_index, _) = storage::abel_supply_state(coin_type);
        let supplier_index = storage::abel_supplier_index(coin_type, supplier);
        storage::update_abel_supplier_state(coin_type, supplier, supply_index);

        let delta_index = supply_index - supplier_index;
        let supplier_tokens = acoin::balance_no_type_args(coin_type, supplier);
        let supplier_delta = (supplier_tokens as u128) * delta_index / Exp_Scale();
        let supplier_accrued = storage::abel_accrued(supplier) + supplier_delta;
        let threshold = if (distribute_all) { 0 } else { constants::Abel_Claim_Threshold() };
        storage::update_abel_accrued(supplier, storage::transfer_abel(supplier, supplier_accrued, threshold));
        storage::emit_distribute_supplier_abel_event(coin_type, supplier, supplier_delta, supply_index);
    }

    fun update_abelcoin_borrow_index_no_type_args(coin_type: String, market_borrow_index: u128) {
        let (borrow_state_index, borrow_state_block) = storage::abel_borrow_state(coin_type);
        let borrow_speed = storage::abel_speed(coin_type);
        let block_number = timestamp::now_seconds();
        ...
        if (delta_blocks > 0 && borrow_speed > 0) {
            ...
            let abel_accrued = (delta_blocks as u128) * borrow_speed;
            let ratio = if (borrow_amount > 0) {
                abel_accrued * Exp_Scale() / borrow_amount
            } else { 0 };
            let new_index = borrow_state_index + ratio;
            storage::update_abel_borrow_state(coin_type, new_index, block_number);
        } else if (delta_blocks > 0) {
            storage::update_abel_borrow_state(coin_type, borrow_state_index, block_number);
        };
    }
    fun distribute_borrower_abelcoin_no_type_args(coin_type: String, borrower: address, market_borrow_index: u128, distribute_all: bool) {
        let (borrow_index, _) = storage::abel_borrow_state(coin_type);
        let borrower_index = storage::abel_borrower_index(coin_type, borrower);
        storage::update_abel_borrower_state(coin_type, borrower, borrow_index);

        let delta_index = borrow_index - borrower_index;
        let borrower_amount = (acoin::borrow_balance_no_type_args(coin_type, borrower) as u128) * Exp_Scale() / market_borrow_index;
        let borrower_delta = borrower_amount * delta_index / Exp_Scale();
        let borrower_accrued = storage::abel_accrued(borrower) + borrower_delta;
        let threshold = if (distribute_all) { 0 } else { constants::Abel_Claim_Threshold() };
        storage::update_abel_accrued(borrower, storage::transfer_abel(borrower, borrower_accrued, threshold));
        storage::emit_distribute_borrower_abel_event(coin_type, borrower, borrower_delta, borrow_index);
    }

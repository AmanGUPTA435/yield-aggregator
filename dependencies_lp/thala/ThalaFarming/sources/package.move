...
module thala_farming::package {
    ...
    //                                                                        //
    const DEPLOYER_ADDRESS: address = @thala_farming_deployer;                //
    const RESOURCE_ACCOUNT_SEED: vector<u8> = b"thala_farming";               //
    const RESOURCE_ACCOUNT_ADDRESS: address = @thala_farming;                 //
    //                                                                        //
    ...
    #[test(thala_farming = @thala_farming)]
    #[expected_failure(abort_code = ERR_PACKAGE_UNAUTHORIZED)]
    fun publish_package_unauthorized_err(thala_farming: &signer) acquires ResourceSignerCapability {
        // setup the manager & package resource
        thala_manager::manager::initialize_for_test(signer::address_of(thala_farming));
        init_for_test();

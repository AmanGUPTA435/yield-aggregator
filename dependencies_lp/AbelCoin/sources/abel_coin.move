module abel_coin::abel_coin {
    ...
    public entry fun initialize(abel_admin: &signer) {
        assert!(signer::address_of(abel_admin) == @abel_coin, error::permission_denied(ENOT_ADMIN));
        ...
        move_to(abel_admin, AdminStore{ admin: @abel_coin });
    ...
    public entry fun change_admin(admin: &signer, new_admin: address) acquires AdminStore {
        only_admin(admin);
        borrow_global_mut<AdminStore>(@abel_coin).admin = new_admin;
    }

    public entry fun delegate_capability<CapType>(admin: &signer, to: address) acquires AdminStore, Delegations {
        only_admin(admin);
        let delegations = &mut borrow_global_mut<Delegations<CapType>>(@abel_coin).inner;
        let i = 0;
    ...
    public entry fun claim_capability<CapType: copy + store>(account: &signer) acquires Delegations, CapStore {
        ...
        let cap = borrow_global<CapStore<CapType>>(@abel_coin).cap;
    ...
    fun is_admin(admin: &signer): bool acquires AdminStore {
        signer::address_of(admin) == borrow_global<AdminStore>(@abel_coin).admin
    }
